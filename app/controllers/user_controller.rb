  ...
  before_action :set_quarter_cutoff, only: [:show, :greensheet]
            ...
            if event.end_time > @quarter_cutoff
                if event.event_type == "Service"
                    if a.attended
                        ...
                        @flakehours += @flake_count*event.hours if @flake_count  < 3
                        @flakehours += 3*event.hours if @flake_count >=3 
                    elsif event.flake_penalty && a.replacement_flaked 
                        @flake_count = @flake_count + 1
                       @flakehours += 0.5*@flake_count*event.hours if @flake_count  < 3
                       @flakehours += 1.5*event.hours if @flake_count >=3 
                    end
    ...
    if request.get?
      ...
      @user.attendances.sort_by {|attend| Event.find(attend.event_id).start_time}.each do |a|
        ...
        if event.event_type == "Service" && hours < 0
             @flake_count = @flake_count + 1
               hours = @flake_count*hours if @flake_count  < 3
               hours = 3*hours if @flake_count >=3     
        end
